...
export START_DATA="$WEEK_BACK_MONTH $WEEK_BACK_DAY $OFF $L $I $BUFFER_LEVELS 1"
...
export START_DATA="$MONTH $DAY $OFF $L $I $BUFFER_LEVELS $C"
...
START=$(eval "printf \"M%dD%dH%dP%dI%dL%dC%d\" $START_DATA")
