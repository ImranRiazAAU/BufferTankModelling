...
void compute_strategy(double *room_temps, int global_t, int off, double *envelope_temps, double *floor_temps, int month,
                      int day, double forward_temp, double *Treturn, int reuse, double consumed_power, double *tank_temps, int tank_levels, double mixing) {

    {
        ...
        fprintf(stderr, "\n MIXING_LEVELS env in connect UPPALL is %s\n", ml_env);
        string mix = ml_env;
        double mixing_levels = std::stod(mix);
        fprintf(stderr, "\n MIX in connect UPPALL is %f\n", mixing_levels);
        ...
        fprintf(stderr, "mixing in connectUppall is %f \n", mixing);
        for(int i=0; i<tank_levels; i++){
            fprintf(stderr, "tank levels connectUppall is %f \n", tank_temps[i]);
        }
        ...
        if (!p.run("./uppaal/bin/verifyta", args)) {
            ...
            p.send(temp.c_str());
            temp = to_string(tank_levels) + "\n";
            p.send(temp.c_str());
            temp = to_string(mixing) + "\n";
            p.send(temp.c_str());
            temp = to_string(mixing_levels) + "\n";
            p.send(temp.c_str());
