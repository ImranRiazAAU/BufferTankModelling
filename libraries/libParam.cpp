...
std::vector<double> tank_temperature;
int global_time;
int offset;
int month;
int day;
...
void _read_param(std::istream &ss) {
    ...
    ss >> tank_levels;
    std::cerr << "tank_levels in libparam is "<<tank_levels<< std::endl;
    ss >> mixing;
    std::cerr << "mixing in libparam is "<<mixing<< std::endl;
    ss >> mixing_levels;
    std::cerr << "mixing levels in libparam is "<<mixing_levels<< std::endl;
    mixing_levels2 = int(mixing_levels);
    tank_temperature.resize(tank_levels, 0);
    ...
    for (size_t i = 0; i < tank_levels; ++i) {
        ss >> tank_temperature[i];
        std::cerr << "tank temp for layer"<<i<<" is "<<tank_temperature[i]<< std::endl;
    }
...
int get_global_time() {
    read_param();
    std::cerr << "RETURNING GTIME " << global_time << std::endl;
    return global_time;
}

int get_offset() {
    read_param();
    std::cerr << "RETURNING OTIME " << offset << std::endl;
    return offset;
}
...
double get_tank_temp(int i) {
    read_param();
    return tank_temperature[i];
}
...
double get_mixing_levels(){
    read_param();
    return (mixing_levels-1.0);
}
int get_mixing_levels_int(){
    read_param();
    return (mixing_levels2-1);
}
