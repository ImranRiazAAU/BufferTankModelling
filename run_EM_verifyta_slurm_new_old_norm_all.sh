...
#SBATCH --exclude=rome0[6-7],naples0[8-9]
#SBATCH --mem=4000
...
export START_DATA="$WEEK_BACK_MONTH $WEEK_BACK_DAY $OFF $L $I $BUFFER_LEVELS $RADIUS $HEIGHT $MIXING_LEVELS 1"
...
export START_DATA="$MONTH $DAY $OFF $L $I $BUFFER_LEVELS $RADIUS $HEIGHT $MIXING_LEVELS $C"
echo "$START_DATA is start data"
export WEIGHT_VALUES="$A $G $Norm_old"
...
START=$(eval "printf \"M%dD%dH%dP%dI%dL%dR%0.2fH%0.2fM%dC%d\" $START_DATA")
